import numpy as np
import matplotlib.pyplot as plt
from matplotlib.patches import FancyArrowPatch
from mpl_toolkits.axes_grid1.anchored_artists import AnchoredSizeBar
import matplotlib.font_manager as fm
from numba import njit, prange
from scipy.ndimage import gaussian_filter
from joblib import Parallel, delayed
from tqdm import tqdm
import multiprocessing as mp
import time
import os

"""
Hydrodynamic Quantum Analogs: 2D Pilot-Wave Simulation
======================================================
Emergence of Born's Rule and phase current from Local Dynamics.


Auteur : Revoire Christian (optimisé par Claude)
Date   : Janvier 2026
"""

# ===============================
# CONFIGURATION IMPORT
# ===============================
try:
    from src.config import Pauli_Config
except ImportError:
    from config import Pauli_Config
CFG = Pauli_Config()

"""
Parameters are loaded from config.py.

To change physics or simulation settings, edit 'src/config.py' 
"""

# Potentiels (désactivés par défaut)
use_potential_h = False
use_potential_c = False
V0 = 0.1
V0_coulomb = 15.0
softening = 1.0

# ===============================
# MOTEUR NUMBA OPTIMISÉ
# ===============================

@njit(parallel=True, fastmath=True)
def evolve_field_2d(psi, psi_new, lap_buffer, x_p, y_p, 
                    dt, dx, dy, omega, gamma, d_psi, 
                    emit_amp, sigma, x_min, y_min, Nx, Ny, V_pot):
    """
    Évolution du champ pilote 2D avec buffers pré-alloués.
    Parallélisation sur les lignes i.
    """
    # 1. Laplacien 2D (parallélisé)
    for i in prange(1, Nx-1):
        for j in range(1, Ny-1):
            lap_buffer[i,j] = (
                (psi[i+1,j] + psi[i-1,j] + psi[i,j+1] + psi[i,j-1] - 4*psi[i,j]) 
                / dx**2
            )
    
    # 2. Mise à jour champ avec potentiel
    for i in prange(1, Nx-1):
        for j in range(1, Ny-1):
            V_local = V_pot[i, j]
            psi_new[i,j] = psi[i,j] + dt * (
                (d_psi + 1j*omega) * lap_buffer[i,j] 
                - gamma * psi[i,j] 
                - 1j * V_local * psi[i,j]
            )
    
    # 3. Source mobile (cutoff optimisé)
    cutoff = int(5 * sigma / dx)
    ix = int((x_p - x_min) / dx)
    iy = int((y_p - y_min) / dy)
    
    i_min = max(1, ix - cutoff)
    i_max = min(Nx-1, ix + cutoff + 1)
    j_min = max(1, iy - cutoff)
    j_max = min(Ny-1, iy + cutoff + 1)
    
    for i in range(i_min, i_max):
        for j in range(j_min, j_max):
            xi = x_min + i * dx
            yj = y_min + j * dy
            dist2 = (xi - x_p)**2 + (yj - y_p)**2
            psi_new[i,j] += emit_amp * np.exp(-0.5 * dist2 / sigma**2) * dt
    
    return psi_new

@njit(fastmath=True)
def get_guidance_2d(psi, x_p, y_p, x_min, y_min, dx, dy, Nx, Ny, epsilon):
    """
    Guidage avec interpolation bilinéaire améliorée.
    Gestion robuste des sauts de phase 2π.
    """
    ix = int((x_p - x_min) / dx)
    iy = int((y_p - y_min) / dy)
    
    # Sécurité bords
    if not (1 <= ix < Nx-2 and 1 <= iy < Ny-2):
        return 0.0, 0.0, 0.0
    
    # Amplitude locale (interpolée)
    fx = (x_p - (x_min + ix*dx)) / dx
    fy = (y_p - (y_min + iy*dy)) / dy
    
    a00 = np.abs(psi[ix, iy])
    a10 = np.abs(psi[ix+1, iy])
    a01 = np.abs(psi[ix, iy+1])
    a11 = np.abs(psi[ix+1, iy+1])
    amp = (1-fx)*(1-fy)*a00 + fx*(1-fy)*a10 + (1-fx)*fy*a01 + fx*fy*a11
    
    if amp < epsilon:
        return amp, 0.0, 0.0
    
    # Gradient de phase (différences finies avec unwrapping)
    def phase_diff(p1, p2):
        d = np.angle(p1) - np.angle(p2)
        if d > np.pi: 
            d -= 2*np.pi
        elif d < -np.pi: 
            d += 2*np.pi
        return d
    
    gx = phase_diff(psi[ix+1, iy], psi[ix-1, iy]) / (2*dx)
    gy = phase_diff(psi[ix, iy+1], psi[ix, iy-1]) / (2*dy)
    
    return amp, gx, gy

@njit(fastmath=True)
def simulate_particle_2d(x_init, y_init, N_steps, thermalization, subsample,
                         dt, dx, dy, omega, gamma, d_psi, emit_amp, sigma,
                         alpha, D_part, epsilon, x_min, x_max, y_min, y_max, 
                         Nx, Ny, V_pot):
    """
    Simule une particule unique avec son champ pilote.
    Retourne positions échantillonnées + champ accumulé.
    """
    # Initialisation
    psi = np.zeros((Nx, Ny), dtype=np.complex64)
    psi_new = np.zeros_like(psi)
    lap_buffer = np.zeros_like(psi)
    
    x_p, y_p = x_init, y_init
    
    # Stockage
    n_samples = (N_steps - thermalization) // subsample
    positions_x = np.zeros(n_samples, dtype=np.float32)
    positions_y = np.zeros(n_samples, dtype=np.float32)
    psi_acc = np.zeros((Nx, Ny), dtype=np.complex64)
    psi2_acc = np.zeros((Nx, Ny), dtype=np.float32)
    
    sample_idx = 0
    
    # Boucle temporelle
    for t in range(N_steps):
        # Évolution champ
        psi_new = evolve_field_2d(psi, psi_new, lap_buffer, x_p, y_p,
                                  dt, dx, dy, omega, gamma, d_psi,
                                  emit_amp, sigma, x_min, y_min, Nx, Ny, V_pot)
        psi[:, :] = psi_new[:, :]
        
        # Normalisation douce (stabilité)
        norm2 = 0.0
        for i in range(Nx):
            for j in range(Ny):
                norm2 += psi[i,j].real**2 + psi[i,j].imag**2
        norm = np.sqrt(norm2 * dx * dy)
        
        if norm > 10.0:
            factor = 10.0 / norm
            for i in range(Nx):
                for j in range(Ny):
                    psi[i,j] *= factor
        
        # Guidage
        amp, gx, gy = get_guidance_2d(psi, x_p, y_p, x_min, y_min, 
                                      dx, dy, Nx, Ny, epsilon)
        
        amp2 = amp**2
        if amp2 > epsilon**2:
            weight = amp2 / (amp2 + epsilon**2)
            drift_x = alpha * weight * gx
            drift_y = alpha * weight * gy
            
            # Clippage pour stabilité
            if drift_x > 5.0: drift_x = 5.0
            elif drift_x < -5.0: drift_x = -5.0
            if drift_y > 5.0: drift_y = 5.0
            elif drift_y < -5.0: drift_y = -5.0
        else:
            drift_x, drift_y = 0.0, 0.0
        
        # Langevin (over-damped)
        noise_x = np.sqrt(2 * D_part * dt) * np.random.randn()
        noise_y = np.sqrt(2 * D_part * dt) * np.random.randn()
        
        x_p += drift_x * dt + noise_x
        y_p += drift_y * dt + noise_y
        
        # Conditions aux bords (réfléchissants)
        if x_p < x_min + 2*dx: 
            x_p = x_min + 2*dx
        elif x_p > x_max - 2*dx: 
            x_p = x_max - 2*dx
        
        if y_p < y_min + 2*dy: 
            y_p = y_min + 2*dy
        elif y_p > y_max - 2*dy: 
            y_p = y_max - 2*dy
        
        # Accumulation après thermalisation
        if t >= thermalization:
            if (t - thermalization) % subsample == 0:
                positions_x[sample_idx] = x_p
                positions_y[sample_idx] = y_p
                sample_idx += 1
            
            # Accumulation continue du champ
            for i in range(Nx):
                for j in range(Ny):
                    psi_acc[i,j] += psi[i,j]
                    psi2_acc[i,j] += psi[i,j].real**2 + psi[i,j].imag**2
    
    return positions_x, positions_y, psi_acc, psi2_acc

# ===============================
# WORKER PARALLÈLE
# ===============================

def worker_particle_2d(seed, particle_id, x_space, y_space, V_pot):
    """
    Worker exécuté sur un cœur CPU séparé.
    Simule UNE particule 2D et retourne ses statistiques.
    """
    np.random.seed(seed)
    
    # Position initiale (distribution radiale)
    r0 = np.random.uniform(5, 10)
    theta0 = np.random.uniform(0, 2*np.pi)
    x_init = r0 * np.cos(theta0)
    y_init = r0 * np.sin(theta0)
    
    # Simulation
    pos_x, pos_y, psi_acc, psi2_acc = simulate_particle_2d(
        x_init, y_init, N_steps, thermalization, SUBSAMPLE,
        dt, dx, dy, omega, gamma, D_psi, emit_amp, sigma_emit,
        alpha, D_part, epsilon, x_min, x_max, y_min, y_max,
        Nx, Ny, V_pot
    )
    
    # Histogramme 2D (léger)
    hist, _, _ = np.histogram2d(pos_x, pos_y, 
                                bins=[len(x_space), len(y_space)],
                                range=[[x_min, x_max], [y_min, y_max]])
    
    # Sauvegarder quelques trajectoires
    save_traj = (particle_id < 5)  # 5 premières particules
    if save_traj:
        # Sous-échantillonner encore pour ne pas surcharger
        traj = np.stack([pos_x[::10], pos_y[::10]], axis=1)
    else:
        traj = None
    
    return {
        'histogram': hist.astype(np.float32),
        'psi_acc': psi_acc,
        'psi2_acc': psi2_acc,
        'n_samples': len(pos_x),
        'trajectory': traj
    }

# ===============================
# SIMULATION PRINCIPALE
# ===============================

def run_born_simulation_2d():
    n_cores = N_CORES if N_CORES > 0 else max(1, mp.cpu_count() + N_CORES)
    
    # Grilles spatiales
    x_space = np.linspace(x_min, x_max, Nx)
    y_space = np.linspace(y_min, y_max, Ny)
    X, Y = np.meshgrid(x_space, y_space, indexing='ij')
    
    # Potentiels
    V_harm = 0.5 * V0 * (X**2 + Y**2) if use_potential_h else np.zeros((Nx, Ny))
    V_coulomb = (-V0_coulomb / np.sqrt(X**2 + Y**2 + softening**2) 
                 if use_potential_c else np.zeros((Nx, Ny)))
    V_pot = V_harm + V_coulomb
    
    # Estimation mémoire
    n_samples_per_particle = (N_steps - thermalization) // SUBSAMPLE
    memory_per_particle_mb = (
        (n_samples_per_particle * 2 * 4 +  # positions (float32)
         Nx * Ny * 8 +  # psi_acc (complex64)
         Nx * Ny * 4)   # psi2_acc (float32)
        / (1024**2)
    )
    total_memory_mb = memory_per_particle_mb * N_runs / n_cores
    
    # Stabilité CFL
    cfl = omega * dt / dx**2
    
    print("="*70)
    print("SIMULATION 2D BORN RULE - VERSION OPTIMISÉE JOBLIB")
    print("="*70)
    print(f"Configuration :")
    print(f"  - Grille : {Nx}×{Ny} ({Nx*Ny:,} points)")
    print(f"  - Cœurs CPU : {n_cores}/{mp.cpu_count()}")
    print(f"  - Particules : {N_runs}")
    print(f"  - Steps/particule : {N_steps:,}")
    print(f"  - Sous-échantillonnage : 1/{SUBSAMPLE}")
    print(f"  - Mémoire estimée : {total_memory_mb:.1f} MB")
    print(f"\nStabilité numérique :")
    print(f"  - CFL = {cfl:.4f} (doit être < 0.5)")
    print(f"  - dx = {dx:.3f}, dy = {dy:.3f}, dt = {dt}")
    print(f"\nParamètres physiques :")
    print(f"  ω={omega}, γ={gamma}, D_ψ={D_psi}")
    print(f"  α={alpha}, D_part={D_part} {'✓ ACTIF' if D_part > 0 else '⚠️ DÉSACTIVÉ'}")
    print(f"  emit_amp={emit_amp}, σ={sigma_emit:.2f}")
    print(f"\nPotentiels :")
    print(f"  Harmonique : {'OUI' if use_potential_h else 'NON'}")
    print(f"  Coulombien : {'OUI' if use_potential_c else 'NON'}")
    print("="*70)
    
    if cfl >= 0.5:
        print("⚠️  ATTENTION : CFL > 0.5, instabilité numérique possible !")
    
    start_time = time.time()
    
    # ========================================
    # PARALLÉLISATION JOBLIB
    # ========================================
    print("\n🚀 Lancement des simulations parallèles...\n")
    
    results = Parallel(n_jobs=n_cores, backend='loky', verbose=0)(
        delayed(worker_particle_2d)(
            seed=42 + p*1000,
            particle_id=p,
            x_space=x_space,
            y_space=y_space,
            V_pot=V_pot
        ) for p in tqdm(range(N_runs), desc="Particules")
    )
    
    elapsed = time.time() - start_time
    print(f"\n✓ Simulation terminée en {elapsed/60:.2f} min")
    print(f"  Vitesse : {N_runs * N_steps / elapsed / 1e6:.2f}M steps/sec")
    
    # ========================================
    # AGRÉGATION DES RÉSULTATS
    # ========================================
    print("\n📊 Agrégation des statistiques...")
    
    rho = np.zeros((Nx, Ny))
    psi_acc = np.zeros((Nx, Ny), dtype=np.complex128)
    psi2_acc = np.zeros((Nx, Ny))
    total_samples = 0
    trajectories = []
    
    for res in tqdm(results, desc="Fusion"):
        rho += res['histogram']
        psi_acc += res['psi_acc']
        psi2_acc += res['psi2_acc']
        total_samples += res['n_samples']
        
        if res['trajectory'] is not None:
            trajectories.append(res['trajectory'])
    
    # Normalisation
    rho_smooth = gaussian_filter(rho, sigma=1.5)
    rho_smooth /= np.trapz(np.trapz(rho_smooth, x_space, axis=0), y_space)
    
    psi_eff = psi_acc / total_samples
    born = psi2_acc / total_samples
    born /= np.trapz(np.trapz(born, x_space, axis=0), y_space)
    
    return X, Y, x_space, y_space, rho_smooth, born, psi_eff, trajectories

# ===============================
# ANALYSE
# ===============================

def analyze_results(X, Y, x_space, y_space, rho, born, psi_eff):
    """
    Calcule corrélation et courant de probabilité.
    """
    # Corrélation
    mask = (rho > 1e-6) & (born > 1e-6)
    if np.sum(mask) > 100:
        corr = np.corrcoef(rho[mask], born[mask])[0,1]
    else:
        corr = 0.0
    
    # Erreur L1
    error_L1 = 0.5 * np.trapz(np.trapz(np.abs(rho - born), x_space, axis=0), y_space)
    
    # Courant de probabilité (polarisation vectorielle)
    grad_y, grad_x = np.gradient(psi_eff)
    J_x = np.imag(np.conj(psi_eff) * grad_x)
    J_y = np.imag(np.conj(psi_eff) * grad_y)
    
    # Moment angulaire moyen
    Lz_field = X * J_y - Y * J_x
    Lz_mean = np.trapz(np.trapz(Lz_field * born, x_space, axis=0), y_space)
    
    print(f"\n{'='*70}")
    print(f"CONVERGENCE VERS |ψ|²")
    print(f"{'='*70}")
    print(f"Corrélation ρ vs |ψ|² : {corr:.4f}")
    print(f"Erreur L¹ : {error_L1:.6f}")
    print(f"<Lz> quantique : {Lz_mean:.3f}")
    
    return corr, error_L1, J_x, J_y

# ===============================
# VISUALISATION
# ===============================

def plot_results_2d_improved(X, Y, rho, born, psi_eff, J_x, J_y, trajectories, corr, error_L1):
    """
    Graphique comparatif 2D
    """
    fig = plt.figure(figsize=(18, 10))
    
    # Extraire grilles spatiales
    x_space = X[:, 0]
    y_space = Y[0, :]
    dx = x_space[1] - x_space[0]
    
    # ====================================================================
    # 1. DENSITÉ PARTICULES (en %)
    # ====================================================================
    ax1 = plt.subplot(2, 3, 1)
    
    # Convertir en pourcentage (rho est normalisée, intégrale = 1)
    # Valeur par pixel = densité × aire_pixel
    rho_percent = rho * 10000  # % par unité d'aire
    
    im1 = ax1.contourf(X, Y, rho_percent, levels=30, cmap='viridis')
    ax1.set_title(r'$\rho(x,y)$ - Densité particules', fontsize=13, fontweight='bold')
    ax1.set_xlabel('x', fontsize=11)
    ax1.set_ylabel('y', fontsize=11)
    
    # Colorbar avec unité explicite
    cbar1 = plt.colorbar(im1, ax=ax1)
    cbar1.set_label('Probabilité (‱ par u.a.²)', fontsize=10)
    ax1.set_aspect('equal')
    
    # Annoter l'intégrale
    ax1.text(0.05, 0.95, r'$\int \rho \, dA = 1$', 
             transform=ax1.transAxes, fontsize=10, va='top',
             bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # ====================================================================
    # 2. BORN RULE (en %)
    # ====================================================================
    ax2 = plt.subplot(2, 3, 2)
    
    born_percent = born * 10000
    
    im2 = ax2.contourf(X, Y, born_percent, levels=30, cmap='viridis')
    ax2.set_title(r'$|\psi|^2$ - Règle de Born', fontsize=13, fontweight='bold')
    ax2.set_xlabel('x', fontsize=11)
    ax2.set_ylabel('y', fontsize=11)
    
    cbar2 = plt.colorbar(im2, ax=ax2)
    cbar2.set_label('Probabilité (‱ par u.a.²)', fontsize=10)
    ax2.set_aspect('equal')
    
    ax2.text(0.05, 0.95, r'$\int |\psi|^2 \, dA = 1$', 
             transform=ax2.transAxes, fontsize=10, va='top',
             bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    # ====================================================================
    # 3. RÉSIDUS (adaptatif)
    # ====================================================================
    ax3 = plt.subplot(2, 3, 3)
    
    diff = rho - born
    vmax = max(np.max(np.abs(diff)), 1e-6)
    
    im3 = ax3.contourf(X, Y, diff*10000, levels=30, cmap='RdBu_r', 
                       vmin=-vmax*10000, vmax=vmax*10000)
    
    # Contours adaptatifs basés sur l'amplitude des résidus
    if vmax > 1e-4:
        # Résidus significatifs → contours larges
        n_contours = 7
        contour_levels = np.linspace(-vmax*0.8, vmax*0.8, n_contours) * 100
        cs = ax3.contour(X, Y, diff * 100, levels=contour_levels, 
                        colors='black', linewidths=0.5, alpha=0.5)
        ax3.clabel(cs, inline=True, fontsize=7, fmt='%.2e')
    else:
        # Résidus très faibles → pas de contours (évite le bruit)
        pass
    
    ax3.set_title(r'$|\psi|^2 - \rho(x,y)$ - Résidus (1e-5)', 
                  fontsize=13, fontweight='bold')
    # ax3.set_title(f'Résidus (r={corr:.4f}, L¹={error_L1:.5f})', fontsize=13, fontweight='bold')    
    ax3.set_xlabel('x', fontsize=11)
    ax3.set_ylabel('y', fontsize=11)
    
    plt.colorbar(im3, ax=ax3)
    ax3.set_aspect('equal')
    
    # ====================================================================
    # 4. COURANT J⃗ 
    # ====================================================================
    ax4 = plt.subplot(2, 3, 4)
    
    step = 2
    X_sub = X[::step, ::step]
    Y_sub = Y[::step, ::step]
    Jx_sub = J_x[::step, ::step]
    Jy_sub = J_y[::step, ::step]
    
    # Fond de densité
    density_bg = np.abs(psi_eff)**2
    ax4.contourf(X, Y, density_bg * 100, levels=20, cmap='gray', alpha=0.3)
    
    # Calcul de l'échelle
    J_norm = np.sqrt(J_x**2 + J_y**2)
    max_J = np.max(J_norm) if np.max(J_norm) > 0 else 1.0
    
    # Quiver plot
    Q = ax4.quiver(X_sub, Y_sub, Jx_sub, Jy_sub, 
                   color='red', alpha=0.8, 
                   scale=max_J*20, width=0.003, headwidth=4)
       
    ax4.set_title(r'Courant $\vec{J}$ (polarisation)', fontsize=13, fontweight='bold')
    ax4.set_xlabel('x', fontsize=11)
    ax4.set_ylabel('y', fontsize=11)
    ax4.set_aspect('equal')
    
    # Note explicative
    ax4.text(0.05, 0.95, r'$\vec{J} = \mathrm{Im}(\psi^* \nabla \psi)$', 
             transform=ax4.transAxes, fontsize=10, va='top',
             bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

    # ====================================================================
    # 5. PHASE (en unités de π)
    # ====================================================================
    ax5 = plt.subplot(2, 3, 5)
    
    phase = np.angle(psi_eff)  # En radians [-π, π]
    phase_pi = phase / np.pi    # En unités de π
    
    # Lissage pour visualisation propre
    from scipy.ndimage import gaussian_filter
    phase_pi_smooth = gaussian_filter(phase_pi, sigma=2.0)
    
    im5 = ax5.contourf(X, Y, phase_pi_smooth, 
                       levels=np.linspace(-1, 1, 31), 
                       cmap='twilight')
    
    ax5.set_title('Phase de l\'onde', fontsize=13, fontweight='bold')
    ax5.set_xlabel('x', fontsize=11)
    ax5.set_ylabel('y', fontsize=11)
    
    # Colorbar avec ticks en π
    cbar5 = plt.colorbar(im5, ax=ax5, ticks=[-1, -0.5, 0, 0.5, 1])
    cbar5.ax.set_yticklabels([r'$-\pi$', r'$-\pi/2$', r'$0$', r'$\pi/2$', r'$\pi$'])
    cbar5.set_label('Phase (rad)', fontsize=10)
    ax5.set_aspect('equal')
    
    # ====================================================================
    # 6. TRAJECTOIRES
    # ====================================================================
    ax6 = plt.subplot(2, 3, 6)
    
    # Trajectoires colorées
    colors = plt.cm.tab10(np.linspace(0, 1, len(trajectories)))
    for traj in trajectories:
        if len(traj) > 0:
            ax6.plot(traj[:,0], traj[:,1], alpha=0.7, lw=1.2)
    ax6.contour(X, Y, rho, levels=8, alpha=0.4, colors='gray')
    ax6.set_title(f'Trajectoires (N={len(trajectories)})', 
                  fontsize=13, fontweight='bold')
    ax6.set_xlabel('x')
    ax6.set_ylabel('y')
    ax6.set_aspect('equal')
    ax6.grid(alpha=0.3)
    
    # ====================================================================
    # TITRE GÉNÉRAL
    # ====================================================================
    fig.suptitle(
        f'Émergence de la règle de Born 2D (N={N_runs}, Steps={N_steps})\n' + 
        f'Corrélation ρ vs |ψ|² : {corr:.4f} | Erreur L¹ : {error_L1:.5f}',
        fontsize=15, fontweight='bold', y=0.98
    )
    
    plt.tight_layout(rect=[0, 0, 1, 0.96])

    # Sauvegarde
    base = f"Born_2D_Improved_N{N_runs}"
    i = 1
    while os.path.exists(f"{base}_V{i}.png"):
        i += 1
    filename = f"{base}_V{i}.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"\n💾 Graphique amélioré sauvegardé : {filename}")
    
    plt.show()

# ====================================================================
# FONCTION PRINCIPALE MODIFIÉE
# ====================================================================
if __name__ == "__main__":
    # Simulation principale
    X, Y, x_space, y_space, rho, born, psi_eff, trajectories = run_born_simulation_2d()
    
    # Analyses
    corr, error_L1, J_x, J_y = analyze_results(X, Y, x_space, y_space, 
                                                rho, born, psi_eff)
    plot_results_2d_improved(X, Y, rho, born, psi_eff, J_x, J_y, 
                                trajectories, corr, error_L1)
    
    print("\n" + "="*70)
    print("✓ SIMULATION 2D TERMINÉE")
    print("="*70)

